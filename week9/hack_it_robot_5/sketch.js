...
function draw() {
    ...
    for (var i = 0; i < robotWidths.length; i++) {
        ...
        //robot's eyes
        fill(255);
        ellipse(-25 * headWidths[i], -robotHeights[i] - 200, eyeWidths[i], eyeHeights[i]);
        ...
        //robots' nose
        fill(noseColors[i]);
        triangle(0, -robotHeights[i] - 190, -noseWidths[i], -robotHeights[i] - 170, noseWidths[i], -robotHeights[i] - 170);
